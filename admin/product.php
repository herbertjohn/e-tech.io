...
function addProductOptions($pdo, $product_id) {
    if (isset($_POST['options'])) {
        ...
        foreach ($list as $option) {
            ...
            $stmt = $pdo->prepare('INSERT INTO products_options (title,name,product_id) VALUES (?,?,?)');
            $stmt->execute([ $option[0], $option[1], $product_id ]);
        }
    }
}
if (isset($_GET['id'])) {
    ...
    foreach($product['options'] as $option) {
        $product['options_string'] .= $option['title'] . '-' . $option['name'] . ',';
    }
    ...
    $page = 'Create';
    if (isset($_POST['submit'])) {
        $stmt = $pdo->prepare('INSERT IGNORE INTO products (name,`desc`,price,rrp,quantity,img,date_added) VALUES (?,?,?,?,?,?,?)');
...
<div class="content-block">
    ...
    <form action="" method="post" class="form responsive-width-100">
        ...
        <div style="display:flex;flex-flow:wrap;">
            ...
            <button id="remove_selected_options">Remover</button>
            <input type="hidden" name="options" value="<?=$product['options_string']?>">
        </div>

        <label for="add_images">Imagens</label>
        <div style="display:flex;flex-flow:wrap;">
...
document.querySelector("#add_option").onclick = function(e) {
    ...
    option.value = document.querySelector("input[name='option_title']").value + '-' + document.querySelector("input[name='option_name']").value;
    option.text = document.querySelector("input[name='option_title']").value + ',' + document.querySelector("input[name='option_name']").value;
